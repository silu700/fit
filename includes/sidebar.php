<div id="sidebar-wrapper">
    <div class="sidebar-heading border-bottom bg-dark text-white">FIT-PRO System</div>
    <div class="list-group list-group-flush">
        <a href="/index.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-tachometer-alt me-2"></i> Dashboard
        </a>
        
        <a href="/calendar/index.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-calendar-check me-2 text-warning"></i> Kalendarz / Grafik
        </a>

        <a href="/users/list.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-users me-2"></i> Użytkownicy
        </a>

        <a href="/groups/list.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-layer-group me-2"></i> Grupy
        </a>

        <a href="/exercises/list.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-dumbbell me-2"></i> Biblioteka Ćwiczeń
        </a>

        <a href="/plans/list.php" class="list-group-item list-group-item-action bg-dark text-white">
            <i class="fas fa-clipboard-list me-2"></i> Plany Treningowe
        </a>
    </div>
</div>
<div id="page-content-wrapper">